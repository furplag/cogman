...
if ! declare -p init_configs >/dev/null 2>&1; then declare -r init_configs='locale selinux ssh sshkey timezone'; fi
...
if ! did_we_have_done; then source <(curl "${repo_url}/el9.initialize.sh" -fLsS); fi
